...
export const UberEatsIcon = ({ size }: { size: number }) => (
...
export const FoodpandaIcon = ({ size }: { size: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
export const OrderTypeBadge = ({ type, size }: { type?: string, size: 'large' | 'small' }) => {
  if (type === '1') {
    return (
      <div className={`flex items-center justify-center rounded-lg bg-[#F3F4F6] text-[#1A1A1A] ${size === 'large' ? 'w-24 h-24' : 'w-10 h-10'}`}>
        <Utensils size={size === 'large' ? 48 : 20} strokeWidth={2.5} />
      </div>
    );
  }
  if (type === '2') {
    return (
      <div className={`flex items-center justify-center rounded-lg bg-[#F3F4F6] text-[#1A1A1A] ${size === 'large' ? 'w-24 h-24' : 'w-10 h-10'}`}>
        <ShoppingBag size={size === 'large' ? 48 : 20} strokeWidth={2.5} />
      </div>
    );
  }
  if (type === '4') {
    return (
      <div className={`bg-[#06C167] text-white flex items-center justify-center rounded-lg shadow-sm ${size === 'large' ? 'w-24 h-24' : 'w-10 h-10'}`}>
        <UberEatsIcon size={size === 'large' ? 48 : 20} />
      </div>
    );
  }
  if (type === '5') {
    return (
      <div className={`bg-[#D70F64] text-white flex items-center justify-center rounded-lg shadow-sm ${size === 'large' ? 'w-24 h-24' : 'w-10 h-10'}`}>
        <FoodpandaIcon size={size === 'large' ? 48 : 20} />
      </div>
    );
  }
  return null;
